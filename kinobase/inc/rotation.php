...
function kb_rotation_settings_page(): void
{
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('Ротация фильмов', 'kinobase'); ?></h1>
        ...
        <div class="notice notice-success is-dismissible">
            <p><?php esc_html_e('Ротация выполнена — фильмы перемешаны.', 'kinobase'); ?></p>
        </div>
        <?php endif; ?>
        ...
        <div style="display:inline-flex;align-items:center;gap:1rem;background:#f0f6fc;border:1px solid #c3d9ed;border-radius:6px;padding:0.75rem 1.25rem;margin-bottom:1.5rem">
            ...
            <span style="color:#555">
                <?php printf(
                    esc_html(_n('минута до следующей ротации', 'минут до следующей ротации', (int) $next_min, 'kinobase')),
                );?>
            </span>
            <?php else : ?>
            <span style="color:#888"><?php esc_html_e('Ротация произойдёт при следующем запросе страницы.', 'kinobase'); ?></span>
            <?php endif; ?>
        </div>
        <?php endif; ?>
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table" role="presentation">
                <tr>
                    <th scope="row"><?php esc_html_e('Включить ротацию', 'kinobase'); ?></th>
                    <td>
                        <label>
                            <input type="checkbox" name="kb_rotation_enabled" value="1"
                                   <?php checked($enabled, 1); ?>>
                            <?php esc_html_e('Перемешивать фильмы на главной и в рубриках', 'kinobase'); ?>
                        </label>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="kb_rotation_interval"><?php esc_html_e('Интервал обновления', 'kinobase'); ?></label>
                    </th>
                    <td>
                        ...
                        <span><?php esc_html_e('минут', 'kinobase'); ?></span>
                        <p class="description">
                            <?php esc_html_e('Минимум 1 мин., максимум 1440 мин. (24 ч.). После изменения интервала нажмите «Сохранить» — ротация произойдёт при следующем истечении таймера.', 'kinobase'); ?>
                        </p>
                    </td>
                </tr>
            </table>
        ...
        <h2 style="font-size:1rem;margin-bottom:0.5rem">
            <?php esc_html_e('Ручное перемешивание', 'kinobase'); ?>
        </h2>
        <p style="color:#555;margin-bottom:0.75rem">
            <?php esc_html_e('Немедленно перемешать фильмы, не дожидаясь истечения таймера.', 'kinobase'); ?>
        </p>
        <form method="post">
            <?php wp_nonce_field('kb_rotation_reset_nonce', 'kb_rotation_reset_nonce_field'); ?>
            <input type="hidden" name="kb_rotation_reset" value="1">
            <?php submit_button(__('Перемешать сейчас', 'kinobase'), 'secondary'); ?>
        </form>
