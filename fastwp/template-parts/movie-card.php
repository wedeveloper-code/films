...
$card_info_parts = [];
if ($card_year_term) {
    $card_info_parts[] = __('Год', 'fastwp') . ': <a href="' . esc_url(get_category_link($card_year_term->term_id)) . '" class="card-meta-link">' . esc_html($card_year) . '</a>';
} elseif ($card_year) {
    $card_info_parts[] = __('Год', 'fastwp') . ': ' . esc_html($card_year);
}
if ($card_quality_term) {
    $card_info_parts[] = __('Качество', 'fastwp') . ': <a href="' . esc_url(get_category_link($card_quality_term->term_id)) . '" class="card-meta-link">' . esc_html($card_quality_cat) . '</a>';
} elseif ($card_quality_cat ?: $quality) {
    $card_info_parts[] = __('Качество', 'fastwp') . ': ' . esc_html($card_quality_cat ?: $quality);
}
    ...
    <div class="movie-gallery" data-href="<?php echo esc_url($permalink); ?>">
        ...
        if ($img_count === 0) {
            ...
            echo '<div style="position:absolute;inset:0;background:#1a1d24;display:flex;align-items:center;justify-content:center;color:#555;font-size:0.75rem;">'
                . esc_html__('Нет изображения', 'fastwp') . '</div>';
        } else {
        ...
        <div class="poster-actions">
            <button
                ...
                aria-label="<?php esc_attr_e('Добавить в избранное', 'fastwp'); ?>"
                title="<?php esc_attr_e('В избранное', 'fastwp'); ?>"
            >
            ...
            <div class="views-badge" title="<?php esc_attr_e('Просмотры', 'fastwp'); ?>">
        ...
        <div class="pricing">
            ...
            <div class="price-tabs" role="tablist">
                <button class="price-tab active" data-tab="rent" role="tab" aria-selected="true">
                    <?php esc_html_e('Аренда', 'fastwp'); ?>
                </button>
                <button class="price-tab" data-tab="buy" role="tab" aria-selected="false">
                    <?php esc_html_e('Покупка', 'fastwp'); ?>
                </button>
            </div>

            <!-- Rent grid -->
            <div class="price-grid" data-type="rent" role="tabpanel">
                <button class="price-cell" title="<?php esc_attr_e('1 просмотр', 'fastwp'); ?>">
                    <?php esc_html_e('1 пр.', 'fastwp'); ?><strong><?php echo esc_html(kb_price($rent1)); ?></strong>
                </button>
                <button class="price-cell" title="<?php esc_attr_e('3 просмотра', 'fastwp'); ?>">
                    <?php esc_html_e('3 пр.', 'fastwp'); ?><strong><?php echo esc_html(kb_price($rent3)); ?></strong>
                </button>
                <button class="price-cell" title="<?php esc_attr_e('5 просмотров', 'fastwp'); ?>">
                    <?php esc_html_e('5 пр.', 'fastwp'); ?><strong><?php echo esc_html(kb_price($rent5)); ?></strong>
                </button>
            </div>

            <!-- Buy grid -->
            <div class="price-grid hidden" data-type="buy" role="tabpanel">
                <button class="price-cell" title="<?php esc_attr_e('1 неделя', 'fastwp'); ?>">
                    <?php esc_html_e('1 нед.', 'fastwp'); ?><strong><?php echo esc_html(kb_price($buy_week)); ?></strong>
                </button>
                <button class="price-cell" title="<?php esc_attr_e('1 месяц', 'fastwp'); ?>">
                    <?php esc_html_e('1 мес.', 'fastwp'); ?><strong><?php echo esc_html(kb_price($buy_month)); ?></strong>
                </button>
                <button class="price-cell" title="<?php esc_attr_e('Навсегда', 'fastwp'); ?>">
            ...
            <div class="coupon-wrap">
                <button class="coupon-btn" data-coupon="<?php echo esc_attr(strtoupper($coupon)); ?>">
                    <?php esc_html_e('Показать купон', 'fastwp'); ?>
                </button>
            </div>
            <?php endif; ?>
