...
while (have_posts()) :
    ...
    <main class="site-content" id="main" role="main">
        ...
        <div class="single-movie">
            ...
            <!-- Breadcrumb -->
            <nav class="single-breadcrumb" aria-label="<?php esc_attr_e('Хлебные крошки', 'fastwp'); ?>">
                <a href="<?php echo esc_url(home_url('/')); ?>"><?php esc_html_e('Главная', 'fastwp'); ?></a>
            ...
            <div class="single-layout">
                ...
                <div class="single-poster-col">
                    <div class="single-poster-viewer" id="single-poster-viewer">
                        <!-- Prev / Next nav zones -->
                        <button class="poster-nav poster-nav-prev" id="poster-prev" aria-label="<?php esc_attr_e('Предыдущее фото', 'fastwp'); ?>">&#8249;</button>
                        <button class="poster-nav poster-nav-next" id="poster-next" aria-label="<?php esc_attr_e('Следующее фото', 'fastwp'); ?>">&#8250;</button>
                        ...
                        <div class="single-indicators">
                            ...
                            <button class="single-indicator<?php echo $idx === 0 ? ' active' : ''; ?>"
                                    ...
                                    aria-label="<?php printf(esc_attr__('Фото %d', 'fastwp'), $idx + 1); ?>"></button>
                            <?php endforeach; ?>
                        </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="single-thumbs" id="single-thumbs">
                        <?php foreach (array_values($gallery_ids) as $idx => $img_id) :
                            ...
                            <button class="single-thumb<?php echo $idx === 0 ? ' active' : ''; ?>"
                                    data-index="<?php echo $idx; ?>"
                                    aria-label="<?php printf(esc_attr__('Фото %d', 'fastwp'), $idx + 1); ?>">
                ...
                <div class="single-info-col">
                    ...
                    <div class="single-title-row">
                        ...
                        <div class="single-title-actions">
                            ...
                            <?php endif; ?>
                            <button class="fav-btn single-fav-btn" data-id="<?php echo esc_attr((string) $post_id); ?>" aria-label="<?php esc_attr_e('В избранное', 'fastwp'); ?>">
                    ...
                    <div class="single-coupon-hero">
                        <button class="coupon-btn coupon-btn-hero"
                            ...
                            🎟 <?php esc_html_e('Показать купон на скидку', 'fastwp'); ?>
                        </button>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="single-pricing">
                        ...
                        <div class="single-price-split">
                            <div class="single-price-col">
                                <div class="single-price-col-header"><?php esc_html_e('Аренда', 'fastwp'); ?></div>
                                <div class="single-price-row">
                                    <span><?php esc_html_e('1 просмотр', 'fastwp'); ?></span>
                                    <strong><?php echo esc_html(kb_price($rent1)); ?></strong>
                                </div>
                                <div class="single-price-row">
                                    <span><?php esc_html_e('3 просмотра', 'fastwp'); ?></span>
                                    <strong><?php echo esc_html(kb_price($rent3)); ?></strong>
                                </div>
                                <div class="single-price-row">
                                    <span><?php esc_html_e('5 просмотров', 'fastwp'); ?></span>
                                    <strong><?php echo esc_html(kb_price($rent5)); ?></strong>
                                </div>
                            </div>
                            <div class="single-price-col">
                                <div class="single-price-col-header"><?php esc_html_e('Покупка', 'fastwp'); ?></div>
                                <div class="single-price-row">
                                    <span><?php esc_html_e('Неделя', 'fastwp'); ?></span>
                                    <strong><?php echo esc_html(kb_price($buy_week)); ?></strong>
                                </div>
                                <div class="single-price-row">
                                    <span><?php esc_html_e('Месяц', 'fastwp'); ?></span>
                                    <strong><?php echo esc_html(kb_price($buy_month)); ?></strong>
                                </div>
                                <div class="single-price-row">
                                    <span><?php esc_html_e('Навсегда', 'fastwp'); ?></span>
            ...
            <div class="single-reviews" id="reviews">
                <h2 class="single-reviews-title">
                    ...
                    printf(
                        esc_html(_n('Отзыв (%d)', 'Отзывы (%d)', (int) $comment_count, 'fastwp')),
                        (int) $comment_count
                    );
                    ?>
                </h2>
                ...
                <div class="review-form-wrap">
                    <h3 class="review-form-heading"><?php esc_html_e('Оставить отзыв', 'fastwp'); ?></h3>
                    <form class="review-form" id="review-form" novalidate>
                        ...
                        <input type="hidden" name="nonce" value="<?php echo esc_attr(wp_create_nonce('fastwp_review')); ?>">

                        <div class="review-fields">
                            <div class="review-field">
                                <label class="review-label" for="review-author"><?php esc_html_e('Имя', 'fastwp'); ?></label>
                                <input type="text" id="review-author" name="review_author" class="review-input"
                                       placeholder="<?php esc_attr_e('Ваше имя', 'fastwp'); ?>"
                                       maxlength="100" required
                                       value="<?php echo is_user_logged_in() ? esc_attr(wp_get_current_user()->display_name) : ''; ?>">
                            </div>
                            <div class="review-field">
                                <label class="review-label" for="review-rating-select"><?php esc_html_e('Оценка', 'fastwp'); ?></label>
                        ...
                        <button type="submit" class="review-submit" id="review-submit">
                            <span class="review-submit-text"><?php esc_html_e('Отправить отзыв', 'fastwp'); ?></span>
            ...
            $related_blocks = [
                [
                    'title' => __('Сейчас смотрят', 'fastwp'),
                    'args'  => [
    ...
    <!-- Lightbox overlay -->
    <div class="kb-lightbox" id="kb-lightbox" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e('Просмотр фото', 'fastwp'); ?>">
        <button class="kb-lb-close" id="kb-lb-close" aria-label="<?php esc_attr_e('Закрыть', 'fastwp'); ?>">✕</button>
        <span class="kb-lb-counter" id="kb-lb-counter"></span>

        <div class="kb-lb-inner">
            <button class="kb-lb-nav kb-lb-prev" id="kb-lb-prev" aria-label="<?php esc_attr_e('Предыдущее', 'fastwp'); ?>">&#8249;</button>
            <img src="" alt="" class="kb-lb-img" id="kb-lb-img">
            <button class="kb-lb-nav kb-lb-next" id="kb-lb-next" aria-label="<?php esc_attr_e('Следующее', 'fastwp'); ?>">&#8250;</button>
        </div>
