...
{
    $vars = [
        '%название_рубрики%'     => __('Название текущей рубрики (например: «Боевики»)', 'fastwp'),
        '%родительская_рубрика%' => __('Название родительской рубрики (например: «Жанры»). Пусто, если рубрика верхнего уровня.', 'fastwp'),
        '%сайт%'                 => __('Название сайта из настроек WordPress (Параметры → Общие)', 'fastwp'),
    ];
    if ($with_filter) {
        $vars['%фильтр%'] = __('Название термина фильтра (например: «2020», «HD»). Только для шаблонов страниц с фильтром.', 'fastwp');
    }
    ...
    return '<div style="margin-top:0.75rem;border:1px solid #ddd;border-radius:4px;padding:0.75rem 1rem;background:#f9f9f9">'
         ...
         . esc_html__('Поддерживаемые переменные:', 'fastwp')
         . '</strong>'
...
function kb_cat_bottom_desc_field(WP_Term $term): void
{
    ...
    <tr class="form-field">
        <th scope="row">
            <label for="kb_bottom_description"><?php esc_html_e('Текст внизу страницы', 'fastwp'); ?></label>
        </th>
        <td>
            ...
            <p class="description">
                <?php esc_html_e('Отображается внизу страницы рубрики, после списка фильмов. Поддерживает HTML.', 'fastwp'); ?>
            </p>
        </td>
    </tr>
    <?php
...
function kb_cat_seo_fields(WP_Term $term): void
{
    ...
    <tr class="form-field">
        <th scope="row">
            <label for="_kb_cat_h1"><?php esc_html_e('H1 (заголовок страницы)', 'fastwp'); ?></label>
        </th>
        <td>
            <input type="text" id="_kb_cat_h1" name="_kb_cat_h1"
                   value="<?php echo esc_attr($h1); ?>" style="width:100%">
            <p class="description">
                <?php esc_html_e('Оставьте пустым — будет использован глобальный шаблон из Параметры → SEO рубрик.', 'fastwp'); ?>
            </p>
        </td>
    </tr>
    <tr class="form-field">
        <th scope="row">
            <label for="_kb_cat_seo_title"><?php esc_html_e('SEO Title', 'fastwp'); ?></label>
        </th>
        <td>
    ...
    <tr class="form-field">
        <th scope="row">
            <label for="_kb_cat_seo_desc"><?php esc_html_e('SEO Description', 'fastwp'); ?></label>
        </th>
        <td>
...
function kb_cat_seo_settings_page(): void
{
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('SEO шаблоны страниц рубрик', 'fastwp'); ?></h1>
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table" role="presentation">
                <tr>
                    <th scope="row">
                        <label for="kb_cat_h1_tpl"><?php esc_html_e('Шаблон H1', 'fastwp'); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    <th scope="row">
                        <label for="kb_cat_seo_title_tpl"><?php esc_html_e('Шаблон &lt;title&gt;', 'fastwp'); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    <th scope="row">
                        <label for="kb_cat_seo_desc_tpl"><?php esc_html_e('Шаблон Description', 'fastwp'); ?></label>
                    </th>
                    <td>
...
function kb_filter_seo_settings_page(): void
{
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('SEO фильтров', 'fastwp'); ?></h1>
        ...
        <form method="post" style="margin-top:1.5rem">
                ...
                <div class="postbox" style="padding:1rem 1.5rem;margin:0">
                    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;margin-bottom:0.75rem">
                        ...
                        <button type="submit" name="kb_remove_combo" value="<?php echo esc_attr($key); ?>"
                                ...
                                onclick="return confirm('<?php esc_attr_e('Удалить правило?', 'fastwp'); ?>')">
                            ✕
                        </button>
                    </div>
            ...
            <?php submit_button(__('Сохранить изменения', 'fastwp'), 'primary', 'kb_save', false,
            ...
            <div style="max-width:860px;border:1px solid #ddd;border-radius:4px;padding:1.25rem 1.5rem;background:#fafafa">
                <p style="margin:0 0 1rem;color:#555;font-size:.9em">
                    <?php esc_html_e('Отметьте рубрики, составляющие комбинацию:', 'fastwp'); ?>
                </p>
                    ...
                    <tr>
                        ...
                        <td style="padding:4px 0">
                            <input type="text" name="kb_new_h1" class="large-text"
                                   placeholder="<?php esc_attr_e('например: %название_рубрики% %фильтр% года', 'fastwp'); ?>">
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="padding:4px 0">
                            <input type="text" name="kb_new_title" class="large-text"
                                   placeholder="<?php esc_attr_e('например: %название_рубрики% %фильтр% | %сайт%', 'fastwp'); ?>">
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="padding:4px 0">
                            <textarea name="kb_new_desc" rows="2" class="large-text"
                                      placeholder="<?php esc_attr_e('Описание для этой комбинации рубрик', 'fastwp'); ?>"></textarea>
                        </td>
                    </tr>
                </table>
