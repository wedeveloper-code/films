...
add_action('admin_notices', 'fastwp_review_rate_admin_notice');
function fastwp_review_rate_admin_notice(): void
{
    ...
    $file_path = 'wp-content/themes/fastwp/inc/ajax-handlers.php';
    echo '<div class="notice notice-info" style="padding:12px 16px;">'
        . '<strong>FastWP — интервал отзывов:</strong> '
        . esc_html(sprintf(
...
add_action('wp_ajax_fastwp_increment_views',        'fastwp_ajax_increment_views');
add_action('wp_ajax_nopriv_fastwp_increment_views', 'fastwp_ajax_increment_views');

function fastwp_ajax_increment_views(): void
{
    // Verify nonce
    if (!check_ajax_referer('fastwp_views', 'nonce', false)) {
...
add_action('wp_ajax_fastwp_contact',        'fastwp_ajax_contact');
add_action('wp_ajax_nopriv_fastwp_contact', 'fastwp_ajax_contact');

function fastwp_ajax_contact(): void
{
    if (!check_ajax_referer('fastwp_contact', 'nonce', false)) {
...
add_action('wp_ajax_fastwp_review',        'fastwp_ajax_review');
add_action('wp_ajax_nopriv_fastwp_review', 'fastwp_ajax_review');

function fastwp_ajax_review(): void
{
    if (!check_ajax_referer('fastwp_review', 'nonce', false)) {
        wp_send_json_error(['message' => 'Ошибка безопасности. Обновите страницу.'], 403);
    }
