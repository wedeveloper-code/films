...
/* ============================================================
   Register meta boxes
   ============================================================ */
add_action('add_meta_boxes', 'fastwp_register_actor_meta_boxes');

function fastwp_register_actor_meta_boxes(): void
{
    // Actor gallery (photos)
    add_meta_box(
        'fastwp_actor_gallery',
        __('Фотографии актёра', 'fastwp'),
        'fastwp_render_actor_gallery_box',
        'actor',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        'fastwp_actor_social',
        __('Социальные сети', 'fastwp'),
        'fastwp_render_actor_social_box',
        'actor',
        'normal',
        'default'
    );
    ...
    add_meta_box(
        'fastwp_actor_films',
        __('Фильмография (фильмы с этим актёром)', 'fastwp'),
        'fastwp_render_actor_films_box',
        'actor',
        'normal',
        'low'
    );
...
/* ============================================================
   ...
   ============================================================ */
function fastwp_render_actor_gallery_box(WP_Post $post): void
{
    wp_nonce_field('fastwp_save_actor_meta', 'fastwp_actor_nonce');
    ...
    <div class="fastwp-gallery-meta">
        <p style="margin-bottom:10px;color:#666;">
            <?php esc_html_e('Первая фотография — главная (отображается на карточке). Добавьте сколько угодно фото.', 'fastwp'); ?>
        </p>
...
/* ============================================================
   ...
   ============================================================ */
function fastwp_render_actor_social_box(WP_Post $post): void
{
...
/* ============================================================
   ...
   ============================================================ */
function fastwp_render_actor_films_box(WP_Post $post): void
{
    $movies = new WP_Query([
    ...
    if (!$movies->have_posts()) {
        echo '<p style="color:#999;">'
            . esc_html__('Нет привязанных фильмов. Добавьте актёра в фильм через страницу редактирования фильма.', 'fastwp')
            . '</p>';
        return;
    }
...
/* ============================================================
   ...
   ============================================================ */
function fastwp_render_cast_box(WP_Post $post): void
{
    wp_nonce_field('fastwp_save_cast', 'fastwp_cast_nonce');
    ...
    <p style="color:#666;margin-bottom:10px;">
        <?php esc_html_e('Начните вводить имя актёра для поиска. Нажмите «Добавить», чтобы привязать.', 'fastwp'); ?>
    </p>
    <div style="display:flex;gap:8px;margin-bottom:12px;">
        <input type="text" id="kb-cast-search" placeholder="<?php esc_attr_e('Имя актёра…', 'fastwp'); ?>"
        ...
        <button type="button" id="kb-cast-add" class="button">
            <?php esc_html_e('Добавить', 'fastwp'); ?>
        </button>
    </div>
...
/* ============================================================
   Save actor meta
   ============================================================ */
add_action('save_post_actor', 'fastwp_save_actor_meta', 10, 2);

function fastwp_save_actor_meta(int $post_id, WP_Post $post): void
{
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    if (!isset($_POST['fastwp_actor_nonce'])
        || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['fastwp_actor_nonce'])), 'fastwp_save_actor_meta')) {
        return;
    }
...
/* ============================================================
   ...
   ============================================================ */
add_action('save_post', 'fastwp_save_cast_meta', 10, 2);

function fastwp_save_cast_meta(int $post_id, WP_Post $post): void
{
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    if (!in_array($post->post_type, ['post', 'movie'], true)) return;
    if (!isset($_POST['fastwp_cast_nonce'])
        || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['fastwp_cast_nonce'])), 'fastwp_save_cast')) {
        return;
    }
    if (!current_user_can('edit_post', $post_id)) return;
