...
add_filter('wp_handle_upload', 'fastwp_optimize_uploaded_image');
 ...
 */
function fastwp_optimize_uploaded_image(array $upload): array
{
    // Only process images
    ...
    $image = fastwp_gd_create($file, $mime);
    if (!$image) {
        return $upload;
    }
    ...
    if (!$saved || (file_exists($webp_file) && filesize($webp_file) > $max_bytes)) {
        $image = fastwp_scale_image($image, $max_bytes, $webp_file);
    }
 ...
 */
function fastwp_gd_create(string $file, string $mime): \GdImage|false
{
    return match ($mime) {
 ...
 */
function fastwp_scale_image(\GdImage $src, int $max_bytes, string $dest): \GdImage
{
    $w = imagesx($src);
    $h = imagesy($src);
