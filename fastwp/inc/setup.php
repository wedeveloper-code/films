...
// ─────────────────────────────────────────────────────────────────
add_action('init', 'fastwp_add_rewrite_rules');
add_action('after_switch_theme', function (): void {
    fastwp_add_rewrite_rules();
    flush_rewrite_rules();
});

function fastwp_add_rewrite_rules(): void
{
...
add_action('pre_get_posts', 'fastwp_filter_archive');

function fastwp_filter_archive(WP_Query $query): void
{
 ...
 */
function fastwp_nav_with_dropdowns(): void
{
    static $done = false;
    if ($done) return;
    $done = true;
    ...
    $genre_parent = fastwp_get_genre_parent();
    ...
    foreach ($main_cats as $mc) {
        ...
        if (!empty($years)) {
            ...
            echo '<div class="nav-dropdown-heading">' . esc_html__('Год', 'fastwp') . '</div>';
            foreach ($years as $term) {
                if (is_wp_error($term)) continue;
                $href = $is_current
                    ? fastwp_filter_url($mc['slug'], $term->slug, $active_genre)
        ...
        if (!empty($genres)) {
            echo '<div class="nav-dropdown-col">';
            echo '<div class="nav-dropdown-heading">' . esc_html__('Жанр', 'fastwp') . '</div>';
            foreach ($genres as $term) {
                if (is_wp_error($term)) continue;
                $href = $is_current
                    ? fastwp_filter_url($mc['slug'], $active_year, $term->slug)
                    : get_category_link($term->term_id);
