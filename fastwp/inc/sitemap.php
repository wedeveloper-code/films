...
function kb_sitemap_maybe_flush(): void
{
    ...
    if ($stored_ver !== FASTWP_VERSION) {
        flush_rewrite_rules(false);
        update_option('kb_sitemap_rw_ver', FASTWP_VERSION);
    }
}
...
add_filter('cron_schedules', static function (array $schedules): array {
    $schedules['biweekly'] = [
        ...
        'display'  => __('Раз в 2 недели', 'fastwp'),
    ];
    return $schedules;
});
...
function kb_sitemap_settings_page(): void
{
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('Карта сайта (sitemap.xml)', 'fastwp'); ?></h1>

        <p>
            <?php esc_html_e('URL карты сайта:', 'fastwp'); ?>
        ...
        <?php if ($notice === 'saved') : ?>
        <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Настройки сохранены. Карта сайта будет пересоздана при следующем запросе.', 'fastwp'); ?></p></div>
        <?php elseif ($notice === 'regenerated') : ?>
        <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Карта сайта успешно обновлена.', 'fastwp'); ?></p></div>
        <?php endif; ?>

        <form method="post" style="max-width:680px;margin-top:1rem">
            <?php wp_nonce_field('kb_sitemap_save', 'kb_sitemap_nonce'); ?>

            <h2 style="font-size:1rem"><?php esc_html_e('Включить в карту сайта', 'fastwp'); ?></h2>
            <table class="form-table">
                <tr>
                    <th><?php esc_html_e('Типы контента', 'fastwp'); ?></th>
                    <td>
                        <?php foreach ([
                            'kb_sitemap_movies' => __('Фильмы (записи и CPT movie)', 'fastwp'),
                            'kb_sitemap_actors' => __('Актёры', 'fastwp'),
                            'kb_sitemap_cats'   => __('Рубрики', 'fastwp'),
                            'kb_sitemap_pages'  => __('Страницы', 'fastwp'),
                        ] as $field => $label) :
                ...
                <tr>
                    <th><label for="kb_sitemap_exc_pages"><?php esc_html_e('Исключить страницы (ID)', 'fastwp'); ?></label></th>
                    <td>
                        <input type="text" id="kb_sitemap_exc_pages" name="kb_sitemap_exc_pages"
                               value="<?php echo esc_attr($exc_ids); ?>" class="regular-text">
                        <p class="description"><?php esc_html_e('ID страниц через запятую (например: 2, 15, 42)', 'fastwp'); ?></p>
                    </td>
                </tr>
            </table>

            <h2 style="font-size:1rem;margin-top:1.5rem"><?php esc_html_e('Автоматическое обновление', 'fastwp'); ?></h2>
            <table class="form-table">
                <tr>
                    <th><label for="kb_sitemap_interval"><?php esc_html_e('Интервал обновления', 'fastwp'); ?></label></th>
                    <td>
                        <select id="kb_sitemap_interval" name="kb_sitemap_interval">
                            <?php foreach ([
                                'manual'   => __('Только вручную', 'fastwp'),
                                'hourly'   => __('Каждый час', 'fastwp'),
                                'twicedaily' => __('Два раза в день', 'fastwp'),
                                'daily'    => __('Каждый день', 'fastwp'),
                                'weekly'   => __('Каждую неделю', 'fastwp'),
                                'biweekly' => __('Раз в 2 недели', 'fastwp'),
                            ] as $val => $label) : ?>
            ...
            <div style="display:flex;gap:1rem;margin-top:1.5rem;align-items:center">
                <?php submit_button(__('Сохранить настройки', 'fastwp'), 'primary', 'kb_sitemap_submit', false); ?>
                <button type="submit" name="kb_sitemap_now" value="1" class="button button-secondary">
                    <?php esc_html_e('Обновить сейчас', 'fastwp'); ?>
                </button>
            </div>
        </form>
    </div>
